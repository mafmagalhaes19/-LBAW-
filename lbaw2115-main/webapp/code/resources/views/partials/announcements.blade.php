@foreach ($announcements as $announcement)
    <div class="col m-2 p-2 border border-primary rounded">
        <div class="row">
            ...
            @if (Auth::user()->isadmin || ( (!is_null($current_role)) &&($current_role->id == $announcement->role_id) ) )
            <div class="col d-flex justify-content-end">
                <!-- admin can't edit -->
                @if (!is_null($current_role))
                    <a class="m-2"
                        href="{{ url('/event/' . $event->id . '/announcement/' . $announcement->id . '/edit/') }}"><svg
                ...
                @endif
                <a class="m-2"
                    href="{{ url('/event/' . $event->id . '/announcement/' . $announcement->id . '/delete/') }}"><svg
                        xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
